...
app.get("/random", (req, res) => {
  res.json(jokes[Math.round(Math.random() * 100)]);
});

app.get("/jokes/:id", (req, res) => {
  res.json(jokes[parseInt(req.params.id)]);
});
...
app.get("/filter", (req, res) => {
  ...
  for (let item of jokes) {
    if (item.jokeType == req.query.type) data.push(item);
  }
...
app.post("/jokes", (req, res) => {
  ...
  res.json({body : req.body, len : jokes.length});
...
app.delete("/jokes/:id", (req, res) => {
  ...
  jokes.splice(parseInt(req.params.id), 1);
